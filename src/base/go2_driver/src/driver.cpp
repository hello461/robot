...
// Header file for publishing odometer messages
#include "nav_msgs/msg/odometry.hpp"
#include "unitree_go/msg/sport_mode_state.hpp"
#include <geometry_msgs/msg/pose_stamped.hpp>
// Publish the header file for coordinate transformation
#include "geometry_msgs/msg/transform_stamped.hpp"
#include "tf2_ros/transform_broadcaster.h"
// Header file for publishing joint status information
#include "sensor_msgs/msg/joint_state.hpp"
#include "unitree_go/msg/low_state.hpp"

using namespace std::placeholders;

// Header file for publishing custom node classes for joint states
class Driver : public rclcpp::Node
{
public:
    Driver() : Node("driver"), body_height_(0.30) 
    {
      RCLCPP_INFO(this->get_logger(), "Driver node creation is used to publish odometer messages, coordinate transformations, and joint status information.");
      ...
      // Coordinate Transformation Broadcaster
      tf_bro_ = std::make_unique<tf2_ros::TransformBroadcaster>(*this);

      // Sports status subscription
...
private:
  ...
  // Subscribe to lower-level state
  rclcpp::Subscription<unitree_go::msg::LowState>::SharedPtr low_state_sub_;
  // Broadcast coordinate transformation
  std::unique_ptr<tf2_ros::TransformBroadcaster> tf_bro_;
  // Subscribing to go2 status
  ...
  {
    body_height_ = state_msg->body_height + 0.057 - 0.046825; // 0.057: The height of base_link when it is LYING DOWN. -0.046825: The radar value is so much lower than base_link.
  }
  ...
  void low_state_cb(const unitree_go::msg::LowState::SharedPtr low_state)
  {
    ...
    // Traverse joint data in low-level state information
  ...
  void pose_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) // This uses `robot_pose`, which is the position of the robot dog's radar, so it needs to be converted into the position of `base_footprint`.
  {
